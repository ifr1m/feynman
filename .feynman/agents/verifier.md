...
thinking: medium
tools: read, bash, grep, find, ls, write, edit, web
output: cited.md
...
2. **Verify every source URL** — use `web` with `action: "goto"` to confirm each URL resolves and contains the claimed content. Read artifact paths with `read`. Flag dead links.
