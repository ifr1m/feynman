...
Use only tool names visible in the current tool set. For web search, call `web` with `action: "search"`; never call `web_search`, `google:search`, `google_search`, or `search_google`.
